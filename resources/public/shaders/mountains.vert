...
// It was difficult finding a suitable starting point, but I think
// this one works OK
...
{
  p *= 5.0;
  vec2 rnd = Noise2(p);//vec2(Hash(floor(p.x*4.0)),
                       //Hash(floor(p.y*4.0)))*.5;
...
vec3 GetClouds(in vec3 sky, in vec3 rd)
{
  ...
  // Uses the ray's y component for horizon fade of fixed colour
  // clouds...
...
void main(void)
{
  ...
  // Use several forward heights, of decreasing influence with
  ...
  #ifdef STEREO
  cameraPos += .45*cu*isCyan; // move camera to the right - the rd
                              // vector is still good
  #endif
  ...
  if( !Scene(cameraPos,rd, distance) )
  ...
  else
    {
      ...
      // Use the distance to sample larger gaps to help stop
      // aliasing...
